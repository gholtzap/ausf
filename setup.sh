...
echo "Setting up AUSF repository..."
...
if [ ! -f .env ]; then
    ...
    echo "  - MONGODB_DATABASE (Database name)"
    ...
    NF_UUID=$(uuidgen | tr '[:upper:]' '[:lower:]')
    ...
    echo "Note: You still need to configure MONGODB_URI and MONGODB_DATABASE in .env"
else
...
echo "  2. Run 'cargo run' to start the AUSF server"
